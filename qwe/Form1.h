...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button2->Text = L"Äîáàâëÿåì ýëåìåíò";
			...
			this->button1->Text = L"Óäàëÿåì ýëåìåíò";
	...
	private:
		...
		void RB_Insert_Fixup(RBT **Node,RBT *newNode){
			while(newNode->parent->color == RED){
				MessageBox::Show("Ïåðåêðàøèâàåì","Ñîîáùåíèå");
				if(newNode->parent == newNode->parent->parent->left){		//ðîäèòåëü íîâîãî óçëà  ÿâëÿåòñÿ 
					RBT *y = newNode->parent->parent->right;				//ïðàâûì èëè ëåâûì ïîòîìêîì
					if(y->color == RED){
						MessageBox::Show("Ïåðåêðàøèâàåì","Ñîîáùåíèå");//öâåò äÿäè íîâîãî óçëà 
						newNode->parent->color = BLACK;				//ñëó÷àé 1 êîãäà äÿäÿ êðàñíûé
						y->color = BLACK;						//ðèñóíîê íà ñòð 349
						newNode->parent->parent->color = RED;
						newNode = newNode->parent->parent;			//------
					} else { if(newNode == newNode->parent->right){
						newNode = newNode->parent;	
						 MessageBox::Show("Ëåâûé ïîâîðîò","Ïîâîðîò");//ñëó÷àé 2 äÿäÿ êðàñíûé è íîâûé óçåë 
						Left_Rotate(Node,newNode);					//ÿâëÿåòñÿ ïðàâûì ïîòîìêîì
					}		//ñòð 350
					newNode->parent->color = BLACK;					//ñëó÷àé 3 äÿäÿ êðàñíûé è íîâûé óçåë 
					newNode->parent->parent->color = RED;	
						 MessageBox::Show("Ïðàâûé ïîâîðîò","Ïîâîðîò");//ÿâëÿåòñÿ ëåâûì ïîòîìêîì
					...
					if(y->color == RED){
						...
						if(newNode == newNode->parent->left){
							newNode = newNode->parent;
						 MessageBox::Show("Ïðàâûé ïîâîðîò","Ïîâîðîò");
							Right_Rotate(Node,newNode);
						}
						newNode->parent->color = BLACK;
						newNode->parent->parent->color = RED;
						MessageBox::Show("Ëåâûé ïîâîðîò","Ïîâîðîò");
	...
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
				 ...
				 for(int i=0; i<(int)vec.size(); i++){
					 ...
					 draw_RBT(tree,500,500,40,2);
						 ...
						 MessageBox::Show("Äàííîãî ýëåìåíòà íå ñóùåñòâóåò!","Îøèáêà!");
					 }
				 }
				 textBox2->Text="";
			 }
